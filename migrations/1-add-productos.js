...
module.exports.id = "add-productos"

module.exports.up = function (done) {
  var log =  this.log
  var Producto = this('productos').collection('productos')
  var OldId = this('oldids').collection('oldids')

  var productsCreation = productosData.map(function(producto) {
    var oldId = producto.id
    var newProducto = {}
    newProducto.name = producto.label
    newProducto.tags.push(producto.tipo)

    return new Promise(function (resolve, reject) {
      Producto.insertOne(newProducto).then(function (r) {
        OldId.insertOne({ n: r.insertedId, o: oldId })
          .then(function(r) {
            resolve()
          })
          .catch(function (err) {
            log('error at OldIds.insertOne')
            log(err)
            reject()
          })
      })
      .catch(function (err) {
        log('error at Producto.insertOne')
        log(err)
        reject()
      })
    })
...
module.exports.down = function (done) {
  ...
  var OldId = this.db.collection('old-ids')

  Producto.deleteMany({})
    .then(function() {
      OldId.deleteMany({})
        .then(function() {
          log('Productos OldId eliminados', err)
          done()
        })
        .catch(function(err) {
          log('OldId delete error', err)
          done()
        })
    })
    .catch(function(err) {
      log('Producto delete error', err)
      done()
    })
}
